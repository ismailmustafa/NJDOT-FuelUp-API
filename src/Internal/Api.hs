...
bridgeDistances coord bridges = zip (haversine coord <$> coords) bridges
  where 
    coords = (\x -> (bridgeLatitude x, bridgeLongitude x)) <$> bridges

-- Calcuate distances to all stations
stationDistances :: (Double, Double) -> [Station] -> [(Double, Station)]
stationDistances c stations = zip (haversine c <$> cs) stations
  where 
    cs = (\x -> (stationLatitude x, stationLongitude x)) <$> stations

-- Sort locations based on haversine distance
getSorted :: [(Double,a)] -> [a]
getSorted list = snd <$> sorted
  where 
    sorted = sortBy (compare `on` fst) list
